...
async function run() {
  ...
  const [admin, manager, receptionist] = await Staff.create([
    { name: 'Alice Admin', email: 'admin@frontdesk.local', role: 'admin', passwordHash: 'seeded' },
    { name: 'Mohan Manager', email: 'manager@frontdesk.local', role: 'manager', passwordHash: 'seeded' },
    { name: 'Riya Reception', email: 'reception@frontdesk.local', role: 'receptionist', passwordHash: 'seeded' }
  ]);
